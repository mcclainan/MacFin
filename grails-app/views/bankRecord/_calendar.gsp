...
<table BORDER=3 CELLSPACING=3 CELLPADDING=3>
	<tr>
		<td COLSPAN="7" class="calendarHead"><B> 
			${utilities.Months.values()[budgetItemInstance.month].name} ${budgetItemInstance.year}
		</B></td>
	</tr>
			...
			<g:each in="${cashFlowWeek.cashFlowDayList}" var="cashFlowDay">
				<td style="text-align: left;">
					...
					<g:if test="${cashFlowDay.isCurrentDay}">
						${balance+=cashFlowDay.ignoreAmount}<br/>
					</g:if>
					<g:if test="${params.controller!='budgetItem'}">
						<g:if test="${balance>0}">
							<span style="color:green">
						</g:if>	
