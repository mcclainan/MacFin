...
<div class="fieldcontain ${hasErrors(bean: bankRecordInstance, field: 'account', 'error')} required">
	...
	<g:select id="account" name="account.id" from="${tracking.Account.list()}" optionKey="id" required="" value="${bankRecordInstance?.account?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: bankRecordInstance, field: 'amount', 'error')} required">
	...
	<g:field name="amount" value="${fieldValue(bean: bankRecordInstance, field: 'amount')}" required=""/>

</div>

<div class="fieldcontain ${hasErrors(bean: bankRecordInstance, field: 'transaction', 'error')} ">
	<label for="transaction">
		<g:message code="bankRecord.transaction.label" default="Transaction" />
		
	</label>
	<g:select id="transaction" name="transaction.id" from="${tracking.Transaction.list()}" optionKey="id" value="${bankRecordInstance?.transaction?.id}" class="many-to-one" noSelection="['null': '']"/>
