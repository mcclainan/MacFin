			...
			<ol class="property-list bankRecord">
				...
				<li class="fieldcontain">
					<span id="transaction-label" class="property-label"><g:message code="bankRecord.transaction.label" default="Transaction" /></span>
						...
						<span class="property-value" aria-labelledby="transaction-label"><g:link controller="transaction" action="show" id="${bankRecordInstance?.transaction?.id}">${bankRecordInstance?.transaction?.encodeAsHTML()}</g:link></span>
					
				</li>
				</g:if>
			
			</ol>
			<g:form url="[resource:bankRecordInstance, action:'delete']" method="DELETE">
				<fieldset class="buttons">
					<g:link class="edit" action="edit" resource="${bankRecordInstance}"><g:message code="default.button.edit.label" default="Edit" /></g:link>
