...
<div class="fieldcontain ${hasErrors(bean: transactionInstance, field: 'account', 'error')} required">
	...
	<g:select id="fromAccountId" name="fromAccountId" from="${tracking.Account.findAllByActive('Y')}" optionKey="id" required="" value="${transactionInstance?.account?.id}" class="many-to-one"/>
</div>
<div class="fieldcontain ${hasErrors(bean: transactionInstance, field: 'account', 'error')} required">
	<label for="account">
		To
	...
	<g:select id="toAccountId" name="toAccountId" from="${tracking.Account.findAllByActive('Y')}" optionKey="id" required="" value="${transactionInstance?.account?.id}" class="many-to-one"/>
</div>
